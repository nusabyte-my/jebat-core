...
from .code_agent import CodeAgent, _detect_project, _build_context_section
...
class BranchAgentManager:
    ...
    async def run_agent_on_branch(
        ...
        if spec.branch_strategy == AgentBranchStrategy.WORKTREE:
            ...
            if self.create_worktree(worktree_path, branch_name, spec.base_branch):
                try:
                    result = await agent_runner(spec, prompt)
    ...
    def create_branch_agent(
        ...
        """Convenience method to create and run an agent on a new branch."""
        spec = AgentSpec(
            name=name,
            role="branch_agent",
            task=task,
        ...
        return self.run_agent_on_branch(spec, task, agent_runner)
