...
class SessionManager:
    ...
    def save_working_memory(
        ...
        now = time.time()
        self.conn.execute(
            "INSERT OR REPLACE INTO working_memory (session_id, goals, facts, constraints, updated_at) "
            "VALUES (?, ?, ?, ?, ?)",
            (session_id, json.dumps(goals), json.dumps(facts), json.dumps(constraints), now),
        )
        self.conn.commit()

    def load_working_memory(self, session_id: str) -> dict | None:
        """Load working memory state for a session. Returns None if not found."""
        row = self.conn.execute(
            "SELECT goals, facts, constraints FROM working_memory WHERE session_id = ?",
            (session_id,),
        ).fetchone()
        if row is None:
            return None
