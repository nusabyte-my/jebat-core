...
SOURCE_ROOT = Path.home() / ".jebat"
TARGET_ROOT = REPO_ROOT / "integrations" / "jebat-gateway"
...
def export_config() -> None:
    source_path = SOURCE_ROOT / "jebat-gateway.json"
    ...
    (TARGET_ROOT / "jebat-gateway.template.json").write_text(json.dumps(exported, indent=2) + "\n")
...
def export_workspace() -> None:
    ...
    source_skill = source_workspace / "skills" / "jebat-agent" / "SKILL.md"
    target_skill = WORKSPACE_TARGET / "skills" / "jebat-agent"
...
def main() -> None:
    ...
    export_config()
    export_workspace()
    print(f"Exported JEBATCore Jebat Gateway bundle to {TARGET_ROOT}")
