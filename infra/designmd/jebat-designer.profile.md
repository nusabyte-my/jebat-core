...
## DESIGNmd CLI (live design-system registry)
Search and pull production design systems from https://designmd.ai straight
from the terminal via the `design` subcommand:
- `jebat design search "dark fintech" --limit 5`   # browse (no key needed)
- `jebat design tags`                              # tag cloud
- `jebat design get <owner/name>`                  # view a system (needs DESIGNMD_API_KEY)
- `jebat design download <owner/name> -o ./DESIGN.md`  # save as DESIGN.md (needs key)
- `jebat design upload ./DESIGN.md --name "My Kit" --tags dark,saas`  # publish (needs key)

Set the key once: `export DESIGNMD_API_KEY=*** (get one at
https://designmd.ai/api-keys). Browsing/search/tags work without it;
get/download/upload require it (enforced by the upstream CLI).
When you download a system, drop it into `vault/design-systems/` and reference
it here so future sessions reuse it.
